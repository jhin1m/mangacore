...
class Author extends Model implements TaxonomyInterface, Cacheable, SeoInterface
{
    use CrudTrait;
    use Sluggable;
    use HasUniqueName;
    ...
    */
    ...
    {
        $site_routes = setting('site_routes_authors', '/tac-gia/{author}');
        if (strpos($site_routes, '{author}')) return 'slug';
    ...
    public function getUrl()
    {
        $params = [];
        $site_routes = setting('site_routes_authors', '/tac-gia/{author}');
        if (strpos($site_routes, '{author}')) $params['author'] = $this->slug;
        if (strpos($site_routes, '{id}')) $params['id'] = $this->id;
        return route('authors.manga.index', $params);
    }

    protected function titlePattern(): string
    {
        return Setting::get('site_author_title', '');
    }

    protected function descriptionPattern(): string
    {
        return Setting::get('site_author_des', '');
    }

    protected function keywordsPattern(): string
    {
        return Setting::get('site_author_key', '');
    }
